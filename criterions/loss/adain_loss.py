...
def style_loss(source: torch.Tensor, target: torch.Tensor):
    assert not target.requires_grad
    ...
    return F.mse_loss(src_mean, trg_mean) + F.mse_loss(src_std, trg_std)
...
def style_loss_dict(source: dict, target: dict):
    ...
    for key in union_keys:
        loss += style_loss(source[key], target[key]).mean()
